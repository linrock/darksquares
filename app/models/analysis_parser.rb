# Converts raw stockfish position analysis output into a hash
#
class AnalysisParser
  ...
  def to_h
    ...
    multipv = @raw_analysis[/multipv (\d+)/, 1]
    if multipv.to_i > 1
      handle_multipv
    else
      handle_singlepv
  ...
  def game_over?
    !!@raw_analysis[/^bestmove \(none\)$/] rescue binding.pry
  end
  ...
  # Parse the raw analysis for a singlepv analysis
  #
  def handle_singlepv
    variations = []
    depths = Set.new
    @raw_analysis.strip.split("\n").reverse.each do |row|
      ...
      unless depths.include? variation[:depth]
        variations << variation
        depths.add variation[:depth]
      end
    end
    @analysis = {
